...
export const mountWorkflow = (root, { fields, templates, prefix, validate, signal }) => {
  ...
  const draw = (focus = true) => {
    ...
    if (!summary.hidden) {
      ...
      for (const field of fields.filter(field => state.errors[field.id])) {
        ...
        listen(link, "click", event => { event.preventDefault(); document.getElementById(fieldControlId(prefix, field)).focus(); });
  ...
  listen(form, "submit", event => { event.preventDefault(); state = workflowTransition(state, { type: "submit", values: readFieldValues(fieldset, fields, prefix) }, fields, validate); draw(); });
  listen(fieldset, "input", () => { state = workflowTransition(state, { type: "input", values: readFieldValues(fieldset, fields, prefix) }, fields, validate); });
  ...
  return { getState: () => structuredClone(state), destroy: () => controller.abort() };
};
