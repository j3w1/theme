...
export function actions(root, { on }) {
  on(root, "click", event => {
    ...
    if (!root.emit("action", { action: button.dataset.action ?? button.value ?? "", pressed: button.getAttribute("aria-pressed") === "true" })) event.preventDefault();
  });
  return {};
}
...
export function fields(root, { on, signal }) {
  ...
  on(input, "keydown", event => {
    if (event.key === "Escape" && input.type === "search" && input.value) { event.preventDefault(); event.stopPropagation(); clear(); }
    if (input.type === "number" && event.shiftKey && ["ArrowUp", "ArrowDown"].includes(event.key)) { event.preventDefault(); step(event.key === "ArrowUp" ? 10 : -10); }
  });
  ...
  return {
    clear, stepBy: step,
    get indeterminate() { return Boolean(input.indeterminate); },
    set indeterminate(value) { if (input.type === "checkbox") input.indeterminate = Boolean(value); },
    get values() { return input.multiple ? [...input.selectedOptions].map(option => option.value) : all(root, 'input[type="checkbox"]:checked').map(box => box.value); },
...
export function switchControl(root, { on, signal }) {
  const button = root.querySelector('[role="switch"]');
  if (!button) return {};
  const initial = root.hasAttribute("checked") || button.getAttribute("aria-checked") === "true";
  let defaultChecked = button.dataset.defaultChecked === undefined ? initial : button.dataset.defaultChecked === "true";
  button.dataset.defaultChecked = String(defaultChecked);
  let checked = initial;
  let successful = root.querySelector("input[data-switch-value]");
  if (!successful) { successful = make(root, "input", { type: "hidden", "data-switch-value": "" }); root.append(successful); }
  const sync = () => {
  ...
  on(button, "click", () => { if (enabled(button)) { checked = !checked; sync(); root.emit("change", { checked, value: successful.value, name: successful.name }); } });
  on(button.form, "reset", () => queueMicrotask(() => { if (!signal.aborted) { checked = defaultChecked; sync(); } }));
  sync();
  return { get checked() { return checked; }, set checked(value) { checked = Boolean(value); sync(); }, attributeChanged(name) { if (name === "checked") { defaultChecked = checked = root.hasAttribute("checked"); button.dataset.defaultChecked = String(defaultChecked); } sync(); } };
}

export function fileInput(root, { on, signal }) {
  const input = root.querySelector('input[type="file"]');
  if (!input) return {};
  let list = root.querySelector(".file-input-list");
  if (!list) { list = make(root, "ul", { class: "file-input-list", "aria-label": "Chosen files" }); root.append(list); }
  ...
  const draw = () => {
    ...
    for (const [index, file] of files.entries()) {
      ...
      const remove = make(root, "button", { type: "button", class: "file-input-remove", "aria-label": `Remove ${file.name}`, "data-remove-index": index }, "✕");
  ...
  const assign = files => {
    ...
    input.files = transfer.files; draw(); nativeInput(input); nativeInput(input, "change");
  };
  on(list, "click", event => {
  ...
  on(list, "keydown", event => { if (["Delete", "Backspace"].includes(event.key) && event.target.matches("button")) { event.preventDefault(); event.target.click(); } });
  const zone = root.querySelector(".file-input-zone");
  if (zone) {
    on(zone, "dragover", event => { if (enabled(input)) { event.preventDefault(); zone.dataset.dragging = "true"; } });
    on(zone, "dragleave", () => { delete zone.dataset.dragging; });
    on(zone, "drop", event => { event.preventDefault(); delete zone.dataset.dragging; assign([...event.dataTransfer.files]); });
  }
  on(input, "change", draw);
  on(input.form, "reset", () => queueMicrotask(() => { if (!signal.aborted) draw(); }));
  draw();
  return { get files() { return [...input.files]; }, clear() { assign([]); }, attributeChanged: draw };
}
