import { c as make, i as all, o as enabled } from "./choice-BYcDpKUh.js";
//#region packages/ui/src/behaviors/display.js
function avatar(root, { on }) {
	...
	return {
		...
		attributeChanged(name) {
			if (name === "src") update(root.getAttribute("src"));
		}
	};
}
function chart(root) {
	...
	let values = all(tbody, "tr").map((row) => ({
		label: row.cells[0]?.textContent.trim() ?? "",
		value: Number(row.cells[1]?.textContent)
	}));
	const update = (data) => {
		...
		group.setAttribute("class", "chart-series");
		svg.append(group);
		...
		for (const [index, row] of values.entries()) {
			...
			for (const [name, value] of Object.entries({
				x: 10 + index * step + step * .15,
				y: 110 - height,
				width: step * .7,
				height
			})) bar.setAttribute(name, String(value));
			const title = root.ownerDocument.createElementNS(ns, "title");
			title.textContent = `${row.label}: ${row.value}`;
			bar.append(title);
			group.append(bar);
			const tr = make(root, "tr");
			tr.append(make(root, "th", { scope: "row" }, row.label), make(root, "td", {}, row.value));
			tbody.append(tr);
		}
...
function editorSearch(root, { on }) {
	...
	const search = () => {
		...
		for (const char of source) {
			const text = char.toLocaleLowerCase();
		...
		if (query) {
			...
			while ((position = folded.indexOf(query, position)) >= 0) {
				...
				if (!matches.length || start >= matches.at(-1).end) matches.push({
					start,
					end
				});
		...
		for (const match of matches) {
			pre.append(root.ownerDocument.createTextNode(source.slice(cursor, match.start)), make(root, "mark", {}, source.slice(match.start, match.end)));
			cursor = match.end;
		}
		pre.append(root.ownerDocument.createTextNode(source.slice(cursor)));
		if (previous) previous.disabled = !matches.length;
		if (following) following.disabled = !matches.length;
		select(0);
	};
	const next = (direction = 1) => {
		if (!matches.length) return;
		select(current + (direction < 0 ? -1 : 1));
		all(pre, "mark")[current]?.scrollIntoView({
			block: "nearest",
			inline: "nearest"
		});
	};
	...
	on(previous, "click", () => next(-1));
	on(following, "click", () => next());
	search();
	return {
		next,
		get source() {
...
function developerView(root, { on }) {
	on(root, "click", (event) => {
		const action = event.target.closest("button,[data-action]");
		if (action && enabled(action)) root.emit("action", { action: action.dataset.action ?? action.textContent.trim() });
	});
	return { setText(text) {
		let pre = root.querySelector("pre");
		if (!pre) {
			pre = make(root, "pre", {
				class: "j3w1-readonly-text",
				tabindex: "0",
				"aria-label": "Read-only text"
			});
			root.firstElementChild.replaceChildren(pre);
		}
		pre.textContent = String(text);
	} };
}
//#endregion
export { editorSearch as i, chart as n, developerView as r, avatar as t };
