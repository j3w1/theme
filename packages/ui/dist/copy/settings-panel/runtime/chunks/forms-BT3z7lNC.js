...
//#region packages/ui/src/behaviors/forms.js
function wizard(root, { on }) {
	...
	const draw = (focus = false) => {
		steps.forEach((button, index) => {
			...
			all(panels[index], "input,select,textarea").forEach((control) => {
				control.disabled = originalDisabled.get(control) || index > reached || root.hasAttribute("disabled");
			});
		});
		...
		if (focus) {
			const heading = panels[step].querySelector("h2,h3,h4");
			if (heading) {
				heading.tabIndex = -1;
				heading.focus();
			}
	...
	const back = () => {
		if (step && !root.hasAttribute("disabled")) go(step - 1);
	};
	steps.forEach((button, index) => on(button, "click", () => {
		if (enabled(button)) go(index);
	}));
	...
	return {
		...
		cleanup() {
			for (const [control, disabled] of originalDisabled) control.disabled = disabled;
		}
	};
}
function formComposition(root, { on }) {
	...
	const showReview = (focus = true) => {
		...
		for (const [name, value] of values) {
			const row = make(root, "tr");
			row.append(make(root, "th", { scope: "row" }, name), make(root, "td", {}, typeof value === "string" ? value : value.name));
			body.append(row);
		}
		form.hidden = true;
		review.hidden = false;
	...
	for (const form of all(root, "form")) on(form, "submit", (event) => {
		event.preventDefault();
		if (!form.reportValidity()) return;
		if (review) showReview();
		else {
			root.emit("submit", { values: [...new FormData(form)] });
			if (status) status.textContent = "Values passed to the host application.";
		}
	});
	on(root.querySelector("[data-form-edit]"), "click", () => {
		review.hidden = true;
		form.hidden = false;
		form.querySelector("input,select,textarea")?.focus();
	});
	on(root.querySelector("[data-form-confirm]"), "click", () => {
		root.emit("submit", { values });
		if (status) status.textContent = "Reviewed values passed to the host application.";
	});
	if (root.querySelector("[data-start=\"review\"]")) showReview(false);
	return {};
}
//#endregion
export { wizard as n, formComposition as t };
