...
function controlLabelText(labels) {
	const text = (node) => {
...
function makeIcon(root, d, attrs = {}) {
...
function mountTemporal(input) {
	...
	const sync = () => {
		...
		if (labelled) {
			editor.setAttribute("aria-labelledby", labelled);
			editor.removeAttribute("aria-label");
		} else {
			editor.removeAttribute("aria-labelledby");
			editor.setAttribute("aria-label", input.getAttribute("aria-label") || labelText() || (date ? "Date" : "Time"));
		}
		if (!nativeTemporal) {
...
function mountChoice(select) {
	...
	if (multiple) {
		...
		control.append(value, makeIcon(select, "M3.5 6 8 10.5 12.5 6", { class: "j3w1-choice-chevron" }));
		list.hidden = true;
