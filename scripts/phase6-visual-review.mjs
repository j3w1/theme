...
export const buildReview = async () => {
  ...
  const compare = `<header class="review-intro"><p class="eyebrow">J3W1 / LOCAL FOUNDATION STUDY</p><h1>One interface. Three directions.</h1><p>Compare the same Vue composition at identical widths. Changes to scene, query and form values synchronize across all four frames. Candidate navigation and framework code are shared.</p></header><div id="review-app"></div>`;
  ...
  const report = { schemaVersion: 1, baselineDigest: baseline.baselineDigest, specimenDigest, sourceInputs: baseline.inputs, implementationInputs,
    ...
    recommendation: "Compare the current local directions using the full-screen review and numerical evidence together. Candidate names and descriptions belong to their input snapshots. No candidate is selected; recommendations and owner feedback are recorded in the local audit.",
