...
export const renderReport = ({ coverage, subject, evidence = null }) => {
  ...
  for (const component of components) {
    ...
    summaries.push(`<tr><th scope="row"><a href="../reference/#${component === "page" ? "main" : anchorFor.component(component)}">${esc(component)}</a></th><td>${positive}; ${counts.failed} failed; ${counts.skipped} skipped</td><td>${own.filter((r) => r.kind === "manual").length} manual records</td></tr>`);
...
export const writeReport = async (evidence = null, { initialize = false } = {}) => {
  ...
  if (initialize) await fs.writeFile(path.join(directory, "ui.json"), stableJson({ schemaVersion: 1, status: "not run", note: "Packed-consumer evidence is published separately after its exact artifact and protocol gates pass." }));
