...
const packed = JSON.parse(execFileSync(process.execPath, [process.env.npm_execpath, "pack", "--workspace", "@j3w1/ui", "--pack-destination", directory, "--json"], { cwd: repoRoot, encoding: "utf8" }));
/* npm 11 prints an array of packed packages; npm 12 prints an object keyed by
   package name. Both describe the same single workspace package. */
const result = Array.isArray(packed) ? packed[0] : packed["@j3w1/ui"] ?? Object.values(packed)[0];
if (!result?.filename) throw new Error("npm pack --json did not describe the packed @j3w1/ui tarball");
