...
if (process.argv.includes("--publish")) {
  ...
  // and thirteen interaction tests per engine. A filtered run cannot pass as full.
  for (const browser of ["chromium","firefox","webkit"]) if(records.filter(record=>record.environment.browser===browser).length!==21)throw new Error(`Incomplete ${browser} protocol: expected 21 tests`);
  if(records.length!==63 || new Set(records.map(record=>`${record.project}/${record.test}`)).size!==63)throw new Error("Incomplete or duplicated packed-consumer protocol");
