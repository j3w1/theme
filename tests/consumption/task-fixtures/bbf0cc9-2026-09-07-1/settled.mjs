...
await page.goto(pathToFileURL(path.resolve('.cache/task-kit-acceptance/composed-output/result.html')).href);
...
const result={settlement:'Awaited all dialog animations without modifying candidate',violations:axe.violations,incomplete:axe.incomplete.map(x=>({id:x.id,nodes:x.nodes.length})),focusCycle:focus,focusedBody,keyboardScroll:{before,after},body:await page.locator('.dialog-body').evaluate(e=>({tabIndex:e.tabIndex,clientHeight:e.clientHeight,scrollHeight:e.scrollHeight})),opacity:await page.locator('dialog').evaluate(e=>getComputedStyle(e).opacity)};
await page.screenshot({path:'.cache/task-kit-acceptance/composed-review/settled-narrow-modal.png'});await browser.close();fs.writeFileSync('.cache/task-kit-acceptance/composed-review/settled.json',JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify(result,null,2));
